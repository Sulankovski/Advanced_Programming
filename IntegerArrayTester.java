...
final class IntegerArray{
    ...
    public IntegerArray concat(IntegerArray ia){
        int[] spoena=new int[a.length+ ia.length()];
        for(int i=0 ; i<a.length; i++){
            spoena[i]=a[i];
        }
        for(int i=0; i< ia.length(); i++){
            spoena[a.length+i]=ia.getElementAt(i);
        }
        return new IntegerArray(spoena);
    }
...
class ArrayReader{
    public static IntegerArray readIntegerArray(InputStream input){
        ...
        int golemina;
        golemina=scanner.nextInt();
        int[] nova=new int[golemina];
        for(int i=0; i<golemina; i++){
            nova[i]=scanner.nextInt();
        }
